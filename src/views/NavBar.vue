    ...
    >
      <header class="sidebar-header">
        <div class="brand-wrap">
          <button class="expand-btn" @click="toggle" :aria-expanded="expanded" :title="expanded ? 'Collapse' : 'Expand'">
            <span v-if="!expanded" class="icon"><img :src="menuIcon" alt="" aria-hidden="true"/></span>
      ...
      <nav class="nav-list" aria-label="Main navigation">
        <ul>
          <li>
            <a href="#" class="nav-item" :title="!expanded ? 'Home' : ''" @click.prevent="navigateTo('/')">
              <span class="icon">
          ...
          <li>
            <a href="#" class="nav-item" :title="!expanded ? 'Characters' : ''" @click.prevent="navigateTo('/characters')">
              <span class="icon">
          ...
          <li>
            <a href="#" class="nav-item" :title="!expanded ? 'Create' : ''" @click.prevent="navigateTo('/create')">
              <span class="icon">
          ...
          <li>
            <a href="#" class="nav-item" :title="!expanded ? 'Settings' : ''" @click.prevent="navigateTo('/settings')">
              <span class="icon">
...
.sidebar-header { display:flex; align-items:center; padding:12px; border-bottom:1px solid #eee }
.brand-wrap { display:flex; align-items:center; gap:8px }
.brand { margin:0; font-size:1rem }
.expand-btn { background:transparent; border:none; padding:6px; border-radius:6px }
.nav-list { padding: 12px 8px; flex:1; overflow:auto }
.nav-list ul { list-style:none; padding:0; margin:0 }
.nav-list li { margin:6px 0 }
.nav-item { display:flex; align-items:center; gap:12px; padding:8px; border-radius:8px; color:#111; text-decoration:none }
.nav-item .icon { display:inline-flex; width:32px; height:32px; align-items:center; justify-content:center }
.nav-item .label { white-space:nowrap }
.nav-item .icon img { width:20px; height:20px; display:block }
.nav-item:hover, .nav-item:focus { background:#f6f6f6 }
.sidebar-footer { padding:12px 8px; border-top:1px solid #eee }

/* When expanded on mobile, make the panel full-screen */
@media (max-width: 599px) {
  .sidebar.expanded { width: 100%; max-width: 100vw }
}

/* On larger screens the collapsed icon strip remains visible and expanded becomes a side panel */
@media (min-width: 600px) {
  .sidebar.expanded { width: 320px; max-width: 40vw }
}

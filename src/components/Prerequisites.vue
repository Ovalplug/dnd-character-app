...
const formattedPrerequisites = computed(() => {
  ...
  props.prerequisites.forEach(prereq => {
    ...
    if (typeof prereq === 'object' && prereq !== null && 'class' in prereq && typeof prereq.class === 'object') {
      const cls = prereq.class as { name?: string; level?: number };
      // Check for prereqLevel type
      if ('type' in prereq && (prereq as { type?: string }).type === 'prereqLevel' && 'level' in prereq) {
          ...
          prereq.proficiency
            .map((prof: Record<string, string>) => {
              ...
              return Object.entries(prof)
                .map(([key, value]) => `${value.charAt(0).toUpperCase() + value.slice(1)} ${key.charAt(0).toUpperCase() + key.slice(1)}s`)
                .join(', ');
            })
            .join('; ')
        );
  ...
  if (classReqs.length === 1) {
    ...
    classReqStr = classReqs.slice(0, -1).join(' or ') + ' or ' + (classReqs[classReqs.length - 1] ?? '');
  }

  // Return combined array
  return [
    ...(classReqStr ? [classReqStr] : []),
    ...otherReqs
  ];
});
</script>
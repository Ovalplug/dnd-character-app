  ...
  <div class="map-editor">
    ...
    <div class="map-editor__grid-container">
      <div class="map-editor__grid" :style="gridStyle">
        ...
        >
          <span v-if="showCoords" class="map-editor__coord"
            >{{ cell.position.x }},{{ cell.position.y }}</span
          >
        </button>
      </div>
    </div>
...
<script setup lang="ts">
  ...
  import {
    GameMapCell as GameMapCellClass,
    GameMap as GameMapClass,
  ...
  const cells = computed(() => {
    ...
    for (let y = 0; y < props.height; y++) {
      for (let x = 0; x < props.width; x++) {
        const existing = mapData.value.find(
          (c: GameMapCell) => c.position.x === x && c.position.y === y
        );
        if (existing) {
  ...
  function toggleCell(cell: GameMapCell): void {
    const nextType: CellType =
      cell.type === 'empty' ? 'wall' : cell.type === 'wall' ? 'difficult_terrain' : 'empty';

    const existing = mapData.value.findIndex(
      (c: GameMapCell) => c.position.x === cell.position.x && c.position.y === cell.position.y
    );
    ...
    for (let y = 0; y < props.height; y++) {
      for (let x = 0; x < props.width; x++) {
        ...
        if (rand < 0.15) {
          ...
          const newCell = new GameMapCellClass(
            new PositionClass(x, y),
            'difficult_terrain' as CellType
          );
          mapData.value.push(newCell);
        }
      }

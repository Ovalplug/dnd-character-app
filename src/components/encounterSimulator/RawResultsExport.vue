<template>
  <div class="results-export">
    ...
    <div class="results-export__controls">
      <button class="btn btn--primary" @click="exportJSON" :disabled="!result">
        Export JSON
      </button>
      <button class="btn btn--primary" @click="exportCSV" :disabled="!result">
        Export CSV
      </button>
    ...
    <div v-if="result" class="results-export__data">
      ...
      <div class="results-export__section">
        ...
        <div class="results-export__table">
          <div v-for="(comb, idx) in survivorList" :key="`survivor-${idx}`" class="results-export__row">
            <span class="results-export__label">{{ comb.name }}</span>
            <span class="results-export__value">HP: {{ comb.finalHp }}, Damage Dealt: {{ comb.damageDealt }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="results-export__section">
        ...
        <div class="results-export__table">
          <div v-for="(comb, idx) in result.finalCombatants" :key="`stat-${idx}`" class="results-export__row">
            <span class="results-export__label">{{ comb.name }}</span>
            <span class="results-export__value">Kills: {{ comb.kills }}, Damage Taken: {{ comb.damageTaken }}</span>
          </div>
        </div>
      </div>
    ...
    <div v-if="copied" class="results-export__toast">
      Copied to clipboard!
    </div>
  </div>
</template>
...
function exportCSV(): void {
  ...
  for (const comb of props.result.finalCombatants) {
    ...
    rows.push(`"${comb.name}","${comb.team}",${comb.finalHp},${comb.damageTaken},${comb.damageDealt},${comb.kills},${died}`);
  }

...
<script lang="ts" setup>
  ...
  import type { ConditionsRules } from '../../../types';
  import PopOut from '../../PopOut.vue';
  import SingleRuleItem from './SingleRuleItem.vue';

  const props = defineProps<{ conditions: ConditionsRules[] }>();
  const sorted = computed(() => [...props.conditions].sort((a, b) => a.name.localeCompare(b.name)));
  const selected = ref<ConditionsRules | null>(null);
</script>

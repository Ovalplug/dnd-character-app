...
<script lang="ts" setup>
  ...
  const props = defineProps<{ actionRules: ActionRules[] }>();
  const sorted = computed(() => [...props.actionRules].sort((a, b) => a.name.localeCompare(b.name)));
  const selected = ref<ActionRules | null>(null);
</script>

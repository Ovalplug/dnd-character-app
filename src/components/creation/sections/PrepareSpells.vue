        ...
        <tbody>
          ...
          >
            ...
            <td>
              <input
                type="checkbox"
                :checked="isPrepared(spell.name)"
                :disabled="!isPrepared(spell.name) && limit > 0 && modelValue.length >= limit"
...
<script lang="ts" setup>
  ...
  const props = defineProps<{
    /** Spells available to prepare from (all class spells, or spellbook contents) */
    spells: Spell[];
    /** Max number that may be prepared (0 = unlimited) */
    limit: number;
    /** Currently prepared spell names */
    modelValue: string[];
  }>();
  ...
  const activeFilterCount = computed(
    () => selectedLevels.value.length + selectedSchools.value.length
  );
